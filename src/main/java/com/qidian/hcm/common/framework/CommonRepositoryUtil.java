...
public class CommonRepositoryUtil {
     ...
     * 查询sql语句
     */
    private int nativeCountResult(String querySql, String countSql, Set<Map.Entry<String, Object>> entrySet) {
    ...
    private int getRowsTotal(int rowCount, Pageable pager) {
        return rowCount % pager.getPageSize() == 0 ? rowCount / pager.getPageSize() :
                rowCount / pager.getPageSize() + 1;
    }

    /**
     * @param querySql 查询的sql语句
     * @param countSql 个数的sql语句，可以不写
     * @param params   参数map类型
     * @param pageNo   第几页
     * @param pageSize 每页多少条
     * @param <T>      对象的类型
     */
    public <T> Page<T> pageByNative(Class clazz, String querySql, String countSql, Map<String, Object> params,
