...
import static com.qidian.hcm.common.utils.ResultCode.DEACTIVATE_OR_DELETE_POSITION_USED_BY_GRADE_FIRST;
import static com.qidian.hcm.common.utils.ResultCode.GRADE_NOT_EXISTS;
...
public class GradeService extends CommonRepositoryUtil {
    ...
    private GradeDTO convertGradeEntityToResponse(GradeEntity gradeEntity) {
    ...
    public Page<GradeResponse> getGradeList(Integer active, String keyword, Integer pageNo, Integer pageSize) {
        ...
        String select = "SELECT id,name,code,alias,rank,enable_time,deleted,enable,custom_field FROM grade  " +
                "WHERE deleted=0 and enable=:enable ";
        if (!StringUtils.isEmpty(keyword)) {
            select += "and (grade.id like :condition or grade.name like :condition or grade.code like :condition  " +
                    "or grade.rank like :condition or grade.custom_field like :condition )";
    ...
    public void updateGrade(UpdateGradeRequest updateGradeRequest) {
        GradeEntity gradeEntity = gradeRepository.findById(updateGradeRequest.getId()).orElseThrow(
                () -> new BizException(GRADE_NOT_EXISTS));
    ...
    public Map<Long, String> getIdNameMap() {
        ...
        return grades.stream().collect(
                Collectors.toMap(GradeEntity::getId, GradeEntity::getName));
    }

    private void checkPositionUsage(Long id) {
        gradeRepository.findById(id).orElseThrow(() -> new BizException(GRADE_NOT_EXISTS));

        List<PositionEntity> positionEntityList =
                positionRepository.findByGradeIdAndEnableAndDeleted(id, YesNo.YES, YesNo.NO);
