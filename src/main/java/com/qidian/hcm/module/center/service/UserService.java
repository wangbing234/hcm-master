...
public class UserService {
     ...
     * @return
     */
    public String loginByPassword(@RequestBody @Valid LoginRequest loginRequest) {
     ...
     * @return
     */
    public LoginResultDTO loginByVerification(@RequestBody SMSLoginRequest request) {
     ...
     * @return
     */
    public User addNotActiveUser(@RequestBody @Valid AddNotActiveUserRequest request) {
        Optional<User> userOptional = userRepository.findByPhone(request.getPhone());
        User user;
    ...
    /**
     * 更新Employee中的手机号码之后同步更新center.user中的phone
     *
     * @param request
     * @return
     */
    public void updateUserPhone(UpdateUserPhoneRequest request) {
        User user = userRepository.findById(request.getUserId())
                .orElseThrow(() -> new BizException(USER_NOT_EXIST));
    ...
    @Transactional
    public void destroy() {
        tenantDataSourceProvider.dropDataBase(TenantThreadHelper.getToken().getId(),
