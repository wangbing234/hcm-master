...
public class FileService {
    ...
    private void upload(String base64Str, String originFileName) {
        // 创建OSSClient实例
        OSSClient ossClient = new OSSClient(hcmConfig.getAliyunOssEndpoint(),
                hcmConfig.getAliyunOssAccessKeyId(),
                hcmConfig.getAliyunOssAccessKeySecret());
        // 关闭OSSClient
        ossClient.putObject(hcmConfig.getAliyunOssBucketName(), originFileName,
                new ByteArrayInputStream(Base64.decodeBase64(base64Str)));
        ossClient.shutdown();
    }
    ...
    public String getTempUrl(Long fileId) {
        ...
        URL url = ossClient.generatePresignedUrl(hcmConfig.getAliyunOssBucketName(),
                attachment.getFileNameOnOss(), expiration);
        // 关闭OSSClient
        ossClient.shutdown();
        tempUrl = url.getProtocol() + "://" + url.getHost() + url.getFile();
        stringRedisTemplate.opsForValue().set(key, tempUrl, hcmConfig.getAliyunOssTempUrlExpire(),
                TimeUnit.MILLISECONDS);
        return tempUrl;
    }

    private String getThumbTempUrl(Long fileId) {
        ...
        OSSClient ossClient = new OSSClient(hcmConfig.getAliyunOssEndpoint(),
                hcmConfig.getAliyunOssAccessKeyId(),
                hcmConfig.getAliyunOssAccessKeySecret());
        GeneratePresignedUrlRequest request = new GeneratePresignedUrlRequest(hcmConfig.getAliyunOssBucketName(),
                attachment.getFileNameOnOss());
