...
public class MultiTenantEntityManagerFactory {
    ...
    // Configures the Hibernate JPA service with multi-center support enabled.
    @PersistenceContext
    @Primary
    @Bean
    public LocalContainerEntityManagerFactoryBean entityManagerFactory(EntityManagerFactoryBuilder builder) {
        ...
        LocalContainerEntityManagerFactoryBean result = builder.dataSource(dataSource)
                .persistenceUnit(SystemConstant.TENANT_KEY)
                .properties(props)
                .packages("com.qidian.hcm").build();
    ...
    private Properties properties() {
        ...
        String physicalNamingStrategy = env.getProperty("spring.jpa.hibernate.physical_naming_strategy",
