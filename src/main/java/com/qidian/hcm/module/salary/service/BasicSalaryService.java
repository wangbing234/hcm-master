...
public class BasicSalaryService {
    ...
    public void saveSalaryRecord() {
        String cycle = salarySettingService.getSalarySettingValue(SalarySettingEnum.CURRENT_CYCLE_MONTH);
        SalaryRecordMonthly salaryRecordMonthly = salaryRecordMonthlyRepository.findByCycle(cycle)
                .orElseThrow(() -> new BizException(ResultCode.SALARY_NOT_ACCOUNTED));
    ...
    public Page<SalaryRecordMonthlyResponse> getSalaryRecords(FilterSalaryRecordRequest request) {
        ...
        List<SalaryRecordMonthlyResponse> salaryRecordMonthlyResponses =
                Lists.newArrayListWithExpectedSize(pageBean.getContent().size());
        ...
        if (!CollectionUtils.isEmpty(list)) {
            for (EmployeeSalaryMonthlyDTO salaryMonthly : list) {
                ...
                if (byEmployeeNo.isPresent()) {
                    ...
                    List<SalaryReportItemDTO> salaryReportItemDTOS = JSONObject.parseArray(
                            salaryMonthly.getItemsResult(), SalaryReportItemDTO.class);
     ...
     * 获取表头列表
     */
    private List<String> getColumnTitleList() {
     ...
     * 获取表头字段列表
     */
    private List<String> getColumnTitleFieldsList() {
        List<SalaryItem> itemList = salaryItemRepository.findAll();
     ...
     * 由薪资项生成表格title
     */
    private String generateColumnTitle(SalaryItem salaryItem) {
    ...
    public void importSalaryFromExcel(UploadFileRequest uploadFileRequest) {
        ...
        String currentCycleMonth = salarySettingService.getSalarySettingValue(
                SalarySettingEnum.CURRENT_CYCLE_MONTH);
        List<EmployeeSalaryMonthly> collect = array.stream().map(
                e -> handlerImportData((JSONObject) e, currentCycleMonth))
                .collect(Collectors.toList()).stream().filter(n -> !Objects.isNull(n)).collect(Collectors.toList());
        employeeSalaryMonthlyRepository.saveAll(collect);
    }

    /**
     * 处理导入的数据，并转换成EmployeeSalaryMonthly实体类
     */
    private EmployeeSalaryMonthly handlerImportData(JSONObject dto, String date) {
    ...
    /**
     * 将dto去除指定属性，并转换成json字符串
     */
    private String convertToJSONString(String itemResult, JSONObject dto) {
    ...
    public Page<SalaryReportAllResponse> getSalaryMonthReport(String month, FilterSalaryRecordRequest request) {
        ...
        List<SalaryReportAllResponse> salaryRecordMonthlyResponses =
                Lists.newArrayListWithExpectedSize(pageBean.getContent().size());
        if (!pageBean.getContent().isEmpty()) {
            for (EmployeeSalaryMonthly salaryMonthly : pageBean.getContent()) {
                ...
                if (byEmployeeNo.isPresent()) {
                    ...
                    salaryReportAllResponse.setSalaryItemDTO(JSONObject.parseArray(salaryMonthly.getItemsResult(),
                            SalaryReportItemDTO.class));
