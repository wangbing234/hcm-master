...
public class EmployeeSalaryMonthlyService {
    ...
    public List<SalaryItemMonthlyResponse> getSalaryMonthly(Long employeeId) {
        ...
        List<SalaryItemMonthlyResponse> salaryItemMonthlyResponseList = JSON.parseArray(employeeSalaryMonthly
                .getItemsResult(), SalaryItemMonthlyResponse.class);
        if (salaryItemMonthlyResponseList == null) {
            salaryItemMonthlyResponseList = new ArrayList<>();
        }
        for (SalaryItem salaryItem : salaryItemList) {
            SalaryItemMonthlyResponse salaryItemMonthlyResponse =
                    getSalaryItemMonthlyResponse(salaryItemMonthlyResponseList, salaryItem.getId());
     ...
     * 根据ID获取薪资项Response
     *
     * @param salaryItemMonthlyResponseList
     * @param id
     */
    private SalaryItemMonthlyResponse getSalaryItemMonthlyResponse(
            List<SalaryItemMonthlyResponse> salaryItemMonthlyResponseList, Long id) {
     ...
     * 前端实时计算
     */
    public List<SalaryItemAccountingDTO> getEmployeeAccounting(Long employeeId,
                                                               SalaryItemAccountingDTO salaryItemMonthlyRequest) {
        validateOrGetEmployee(employeeId);
        List<SalaryItemAccountingDTO> salaryItemAccountingDTOList = new ArrayList<>();
        //查询当前薪资项
        SalaryItem salaryItemResult = salaryItemRepository.findByCode(salaryItemMonthlyRequest.getCode())
                .orElseThrow(() -> new BizException(ResultCode.SALARY_ITEM_CODE_NOT_EXISTS));

        //计算薪资项
        calculateService.accountingCalculate(employeeId, salaryItemResult, salaryItemMonthlyRequest.getValue())
                .forEach(salaryItem -> {
     ...
     *
     * @param employeeId
     * @param salaryItemMonthlyRequestList
     */
    @Transactional
    public void saveDetailInfo(Long employeeId, List<SalaryItemMonthlyRequest> salaryItemMonthlyRequestList) {
        ...
        EmployeeSalaryMonthly employeeSalaryMonthly = getEmployeeSalaryItemsMonthlyDetail(employeeId);
        List<SalaryItemMonthlyDTO> salaryItemMonthlyDTOList = JSON.parseArray(employeeSalaryMonthly.getItemsResult(),
                SalaryItemMonthlyDTO.class);
        if (salaryItemMonthlyDTOList == null) {
            salaryItemMonthlyDTOList = new ArrayList<>();
        }
        //在保存之前校验数据
        List<SalaryItem> changedSalaryItem = getAttendanceSalaryItem(
                salaryItemMonthlyRequestList, salaryItemMonthlyDTOList);
        for (SalaryItemMonthlyRequest salaryItemMonthlyRequest : salaryItemMonthlyRequestList) {
            SalaryItemMonthlyDTO salaryItemMonthlyDTO = getSalaryItemMonthlyDTO(salaryItemMonthlyDTOList,
                    salaryItemMonthlyRequest.getId());
     ...
     * 考勤变动后计算相关薪资项
     * @param salaryItemMonthlyRequestList
     * @param salaryItemMonthlyDTOList
     */
    private List<SalaryItem> getAttendanceSalaryItem(List<SalaryItemMonthlyRequest> salaryItemMonthlyRequestList,
                                                  List<SalaryItemMonthlyDTO> salaryItemMonthlyDTOList) {
        Map<String, SalaryItemMonthlyRequest> salaryItemMonthlyRequestMap = salaryItemMonthlyRequestList.stream()
                .collect(Collectors.toMap(SalaryItemMonthlyRequest::getCode, salaryItemMonthly -> salaryItemMonthly));
        Map<String, SalaryItemMonthlyDTO> salaryItemMonthlyDTOListMap = salaryItemMonthlyDTOList.stream()
                .collect(Collectors.toMap(SalaryItemMonthlyDTO::getCode, salaryItemMonthlyDTO -> salaryItemMonthlyDTO));
        //查询考勤字典公式
        ...
        salaryItemList.forEach(salaryItem -> {
            ...
            if (Objects.nonNull(salaryItemMonthlyRequest) && Objects.nonNull(salaryItemMonthlyRequest.getValue())) {
                ...
                if (Objects.isNull(salaryItemMonthlyDTO) ||
                        salaryItemMonthlyRequest.getValue().equals(salaryItemMonthlyDTO.getValue())) {
    ...
    public EmployeeSalaryMonthly getEmployeeSalaryItemsMonthlyDetail(Long employeeId) {
        ...
        Optional<EmployeeSalaryMonthly> employeeSalaryMonthlyOptional = employeeSalaryMonthlyRepository
                .findByEmployeeIdAndDate(employeeId, currentCycleMonth);
        ...
        if (employeeSalaryMonthlyOptional.isPresent()) {
            ...
            if (employeeOptional.isPresent()) {
                ...
                if (employee.getStatus() == EmployeeStatus.former) {
                    ...
                    Date startDate = DateUtil.parseDate(currentSalaryCycleTime.getStartTime(),
    ...
    public void salaryExclude(EmployeeListRequest employeeListRequest) {
        ...
        List<EmployeeSalaryMonthly> employeeSalaryMonthlyList = employeeSalaryMonthlyRepository
                .findByEmployeeIdInAndDate(employeeListRequest.getIds(), currentCycleMonth);
     ...
     * 获取员工实体信息，不存在设置默认值
     *
     * @param employeeSalaryMonthlyList
     * @param employeeId
     * @return
     */
    private EmployeeSalaryMonthly getSalaryMonthlyByEmployeeId(List<EmployeeSalaryMonthly> employeeSalaryMonthlyList,
                                                               Long employeeId) {
     ...
     * @return
     */
    private Employee validateOrGetEmployee(Long employeeId) {
