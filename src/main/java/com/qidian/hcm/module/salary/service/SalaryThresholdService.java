...
public class SalaryThresholdService {
    ...
    public void updateThreshold(Long id, SalaryThresholdRequest salaryThresholdRequest) {
        ...
        SalaryThreshold salaryThreshold = salaryThresholdOptional.orElseThrow(
                () -> new BizException(ResultCode.SALARY_THRESHOLD_NOT_EXISTS));
        ...
        if (!CollectionUtils.isEmpty(employeeFinancialList)) {
            calculateService.formulaCalculate(employeeFinancialList.stream()
                    ...
                    .collect(Collectors.toList()), SalaryFormulaIKeyword.THRESHOLD);
        }
    }
}
