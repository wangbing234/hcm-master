...
public class EmployeeFinancialRepositoryImpl {
    ...
    public Page<Employee> getEmployeeSalaryPage(FilterEmployeesSalaryRequest filter, String date) {
        ...
        sb.append(" select e.* from employee as e")
                .append(" left join employee_financial as ef on e.id=ef.employee_id")
                .append(" left join employee_salary_monthly as eam on e.id=eam.employee_id and eam.date=:date  where");
        //如果记薪
        ...
        return commonRepositoryUtil.pageByNative(Employee.class, sb.toString(), null, map,
