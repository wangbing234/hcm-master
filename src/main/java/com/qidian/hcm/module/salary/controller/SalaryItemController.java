...
public class SalaryItemController {
    ...
    @PostMapping
    public Result addSalaryItem(@RequestBody @Valid SalaryItemRequest salaryItemRequest) {
        return genSuccessResult(salaryItemService.addSalaryItem(salaryItemRequest));
    }

    @ApiOperation(value = "编辑薪资项")
    @PutMapping("{id}")
    public Result updateSalaryItem(@PathVariable Long id, @RequestBody @Valid SalaryItemRequest salaryItemRequest) {
    ...
    @DeleteMapping("{id}")
    public Result deleteSalaryItem(@PathVariable Long id) {
