...
public class CustomizedFormsRequest {
    ...
    @NotNull
    private Boolean multiRecord; //是否支持多条记录

    @ApiModelProperty(value = "模块是否必填")
    @NotNull
    private Boolean required; //模块是否必填

    @ApiModelProperty(value = "员工入职是否启用")
    @NotNull
    private Boolean onBoard; //员工入职是否启用

    @ApiModelProperty(value = "索引")
    private Integer idx; //索引

    @ApiModelProperty(value = "模块分类,position岗位信息，basic基本信息,other其他信息")
    private FormType type; //position", //模块分类，position岗位信息，basic基本信息,other其他信息
