...
public class EmployeeCustomizedFormService {
    ...
    public CustomizedFormLayoutResponse getCustomizedFormLayout() {
        List<CustomizedEmployeeForm> employeeForms = customizedEmployeeFormRepository
                .findByEnableOrderByTypeAscIdxAsc(TRUE);
    ...
    public CustomizedFormsResponse getCustomizedForms() {
        ...
        Map<FormType, List<CustomizedEmployeeForm>> groupedForms = forms.stream()
                .sorted(Comparator.comparingInt(CustomizedEmployeeForm::getIdx))
                .collect(groupingBy(CustomizedEmployeeForm::getType));

        return new CustomizedFormsResponse(convertFormListToDTOList(groupedForms.get(FormType.position)),
                convertFormListToDTOList(groupedForms.get(FormType.basic)),
                convertFormListToDTOList(groupedForms.get(FormType.other)));
    }
     ...
     * 构建自定义表单
     *
     * @param customizedFormsRequest
     * @param customizedEmployeeForm
     * @param isUpdated              是否是更新
     */
    private void buildCustomizedForms(CustomizedFormsRequest customizedFormsRequest,
                                      CustomizedEmployeeForm customizedEmployeeForm, Boolean isUpdated) {
        if (isUpdated) {
    ...
    public void updateCustomizedForms(Long id, CustomizedFormsRequest customizedFormsRequest) {
        CustomizedEmployeeForm customizedEmployeeForm = customizedEmployeeFormRepository.findById(id)
        ...
        if (fieldsList.isEmpty()) {
            ...
            List<Long> deleteIds = fieldsList.stream().filter(field -> field.getId() != null)
                    .map(CustomizedFormsFieldsRequest::getId)
                    .collect(toList());
    ...
    public void toggleActiveCustomizedForms(Long id, Boolean enable) {
        customizedEmployeeFormRepository.findById(id)
                .orElseThrow(() -> new BizException(ResultCode.CUSTOMIZED_FORM_IS_NULL));
    ...
    public CustomizedFormResponse getCustomizedFormsDetail(Long id) {
        ...
        List<CustomizedField> list = customizedFieldRespository
                .findByTargetTypeAndTargetIdOrderByIdx(TargetType.employee_form, id);
    ...
    private List<CustomizedEmployeeFormDTO> convertFormListToDTOList(List<CustomizedEmployeeForm> forms) {
    ...
    private List<CustomizedFormResponse> groupCustomizedForm(List<CustomizedEmployeeForm> customizedEmployeeForms,
                                                             FormType formType) {
        ...
        List<FormFieldDTO> formFieldDTOS = customizedFieldService.findByTargetTypeAndTargetIds(
                TargetType.employee_form, ids, entity -> {
                    FormFieldDTO formFieldDTO = new FormFieldDTO();
        ...
        if (!CollectionUtils.isEmpty(formFieldDTOS)) {
            Map<Long, List<FormFieldDTO>> formFieldMap = formFieldDTOS.stream()
                    .collect(groupingBy(FormFieldDTO::getFormId));
