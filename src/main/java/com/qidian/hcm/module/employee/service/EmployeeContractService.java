...
public class EmployeeContractService {
    ...
    public void updateContractProbationEndDate(Long employeeId, Date qualifyDate) {
        ...
        EmployeeContract employeeContract = contracts.stream()
                .filter(ec -> qualifyDate.compareTo(ec.getStartDate()) >= 0
                        && qualifyDate.compareTo(ec.getEndDate()) <= 0)
                .findFirst().orElseThrow(() -> new BizException(ResultCode.NOT_MATCH_QUALIFY_CONTRACT));
