...
public class EmployeeHistoryService {
    ...
    public List<EmployeeHistoryDTO> findEmployeeHistory(Long id) {
        List<EmployeeHistory> employeeHistories = employeeHistoryRepository
                .findAllByEmployeeIdOrderByCreatedDateDesc(id);

        return employeeHistories.stream().map(employeeHistory ->
                new EmployeeHistoryDTO(employeeHistory.getId(), employeeHistory.getContent(),
                        employeeHistory.getRemark(), employeeHistory.getCreatedDate())).collect(toList());
    }

    void saveEmployeeHistory(Long employeeId, String content, Date date, String remark) {
